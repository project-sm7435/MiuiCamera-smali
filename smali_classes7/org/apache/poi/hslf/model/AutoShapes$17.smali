.class final Lorg/apache/poi/hslf/model/AutoShapes$17;
.super Ljava/lang/Object;
.source "SourceFile"

# interfaces
.implements Lorg/apache/poi/hslf/model/ShapeOutline;


# annotations
.annotation system Ldalvik/annotation/EnclosingClass;
    value = Lorg/apache/poi/hslf/model/AutoShapes;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x9
    name = null
.end annotation


# direct methods
.method public constructor <init>()V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method


# virtual methods
.method public getOutline(Lorg/apache/poi/hslf/model/Shape;)Ljava/awt/Shape;
    .locals 12

    const/16 p0, 0x147

    const/16 v0, 0x1518

    invoke-virtual {p1, p0, v0}, Lorg/apache/poi/hslf/model/Shape;->getEscherProperty(SI)I

    move-result p0

    new-instance p1, Ljava/awt/geom/GeneralPath;

    invoke-direct {p1}, Ljava/awt/geom/GeneralPath;-><init>()V

    new-instance v0, Ljava/awt/geom/Arc2D$Float;

    int-to-float v5, p0

    const/high16 v6, 0x43340000    # 180.0f

    const/4 v7, 0x0

    const/4 v1, 0x0

    const/4 v2, 0x0

    const v3, 0x46a8c000    # 21600.0f

    move v4, v5

    const/4 v5, 0x0

    invoke-direct/range {v0 .. v7}, Ljava/awt/geom/Arc2D$Float;-><init>(FFFFFFI)V

    const/4 v9, 0x0

    invoke-virtual {p1, v0, v9}, Ljava/awt/geom/GeneralPath;->append(Ljava/awt/Shape;Z)V

    div-int/lit8 v0, p0, 0x2

    int-to-float v10, v0

    const/4 v11, 0x0

    invoke-virtual {p1, v11, v10}, Ljava/awt/geom/GeneralPath;->moveTo(FF)V

    rsub-int v0, v0, 0x5460

    int-to-float v0, v0

    invoke-virtual {p1, v11, v0}, Ljava/awt/geom/GeneralPath;->lineTo(FF)V

    invoke-virtual {p1}, Ljava/awt/geom/GeneralPath;->closePath()V

    new-instance v1, Ljava/awt/geom/Arc2D$Float;

    rsub-int p0, p0, 0x5460

    int-to-float v3, p0

    const/high16 v7, 0x43340000    # 180.0f

    const/4 v8, 0x0

    move v5, v4

    const v4, 0x46a8c000    # 21600.0f

    invoke-direct/range {v1 .. v8}, Ljava/awt/geom/Arc2D$Float;-><init>(FFFFFFI)V

    move v4, v5

    invoke-virtual {p1, v1, v9}, Ljava/awt/geom/GeneralPath;->append(Ljava/awt/Shape;Z)V

    const p0, 0x46a8c000    # 21600.0f

    invoke-virtual {p1, p0, v0}, Ljava/awt/geom/GeneralPath;->moveTo(FF)V

    invoke-virtual {p1, p0, v10}, Ljava/awt/geom/GeneralPath;->lineTo(FF)V

    new-instance v1, Ljava/awt/geom/Arc2D$Float;

    const/4 v3, 0x0

    const v4, 0x46a8c000    # 21600.0f

    invoke-direct/range {v1 .. v8}, Ljava/awt/geom/Arc2D$Float;-><init>(FFFFFFI)V

    invoke-virtual {p1, v1, v9}, Ljava/awt/geom/GeneralPath;->append(Ljava/awt/Shape;Z)V

    invoke-virtual {p1, v11, v10}, Ljava/awt/geom/GeneralPath;->moveTo(FF)V

    invoke-virtual {p1}, Ljava/awt/geom/GeneralPath;->closePath()V

    return-object p1
.end method
